...
# calculation gt
def calculate_r_factor(estimate_result, fobs_data):
    hkl_gt_list = []
    # R variable
    sum_f_obs_gt, sum_f_calc_gt, sum_fo_fc_diff_gt = 0, 0, 0
    sum_f_obs_x_f_calc_gt = 0, 0
    sum_f_calc_power_gt = 0, 0

    est_data = []
    for er in estimate_result:
        est_data.append([er.intensity.h, er.intensity.k, er.intensity.l, er.fest(), int(f'{er.intensity.h}{er.intensity.k}{er.intensity.l}')])
    est_data.sort(key=lambda x: x[-1])

    for ed, fd in zip(est_data, fobs_data):
        assert ed[0:3] == fd[0:3], f'{ed[0:3]} {fd[0:3]}'
        h, k, l = ed[0:3]
        f_calc_clisalis_value_squared, f_obs_value_squared, f_sigma_squared = fd[3], fd[4], fd[5]
        f_calc_value = ed[3]
